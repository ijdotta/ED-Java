...
/**
 * Map Test - Estructura de Datos (DCIC-UNS).
 * Define casos de pruebas a aplicar sobre el TDA Map<K,V>.
...
public class MapTest {
	...
	/*
	 * Retorna la clase tipo Map a testear.
	 * Indique la clase que implementa Map<K,V> que desea testear.
	 * Por ejemplo: MapOpenHash<Integer,Integer>, MapClosedHash<Integer,Integer> o MapList<Integer,Integer>.
	 */
	private Map<Integer, Integer> getMapeo() {
		return new Mapeo_con_AVL<Integer, Integer>();
	}
	...
	public void size() {
		...
		assertTrue("Tamaño de Mapeo justo después de ser creada != 0", s.size() == 0);
		
		try {
			s.put(i1, n1);
			assertTrue("Tamaño de Mapeo luego de insertar una vez != 1", s.size() == 1);

			s.put(i2, n2);
			assertTrue("Tamaño de Mapeo luego de insertar dos veces != 2", s.size() == 2);

			s.put(i3, n3);
			assertTrue("Tamaño de Mapeo luego de insertar tres veces != 3", s.size() == 3);

			// Testeando que el método remove() actualice el size, y que get(K key) no lo haga.
			s.get(i1);
			assertTrue("Tamaño de Mapeo cambió al invocar get()", s.size() == 3);

			s.remove(i1);
			assertTrue("Tamaño de Mapeo luego de eliminar una entrada es != 2", s.size() == 2);

			s.get(i2);
			assertTrue("Tamaño de Mapeo cambió al invocar get()", s.size() == 2);

			s.remove(i2);
			assertTrue("Tamaño de Mapeo luego de eliminar es != 1", s.size() == 1);

			s.get(i3);
			assertTrue("Tamaño de Mapeo cambió al invocar get()", s.size() == 1);

			s.remove(i3);
			assertTrue("Tamaño de Mapeo luego de eliminar es != 0", s.size() == 0);
	...
	public void isEmpty() {
		try {
			...
			assertTrue("EL Mapeo no está vacío justo después de ser creado",s.isEmpty());
			
			s.put(i1, n1);
			assertTrue("El Mapeo está vacía después de insertar 1 elemento",!s.isEmpty());
			
			s.put(i2, n2);
			assertTrue("El Mapeo está vacía después de insertar 2 elementos",!s.isEmpty());
			
			s.put(i3, n3);
			assertTrue("El Mapeo está vacía después de insertar 3 elementos",!s.isEmpty());
		...
		try {
			...
			s.get(i1);
			assertTrue("El Mapeo está vacío después de invocar get() teniendo 3 elementos", !s.isEmpty());

			s.remove(i1);
			assertTrue("El Mapeo está vacío después de eliminar 1 elemento, quedando 2.", !s.isEmpty());

			s.get(i2);
			assertTrue("El Mapeo está vacío después de invocar get(i2) teniendo 2 elementos", !s.isEmpty());

			s.remove(i2);
			assertTrue("El Mapeo está vacío después de eliminar 1 elemento, quedando 1.", !s.isEmpty());

			s.get(i3);
			assertTrue("El Mapeo está vacío después de invocar get(i3) teniendo 1 elemento", !s.isEmpty());

			s.remove(i3);
			assertTrue("El Mapeo no está vacío después de eliminar todos los elementos que tenía.", s.isEmpty());
	...
	/* _______________________TESTS METODO get(K key)_____________________________*/
	...
	public void get() {
		...
		try {
			...
			valor = s.get(i1);
			assertTrue("El Mapeo debería devolver nulo cuando la clave no está en el mapeo.", valor == null);
	...
	/* _______________________TESTS METODOS put() y remove()_____________________________*/
	...
	public void put_remove() {
		...
		try{
			...
			valor = s.put(clave_aux, valor_aux);
			assertTrue("El valor debería ser nulo", valor == null);
			
			valor = s.put(clave_aux, valor_aux * 10);
			assertTrue("El insertar una clave que ya existía, no retorna el valor reemplazado", valor == valor_aux);
	...
	/* _______________________TESTS METODOS keys(), values(), entries()_____________________________*/
	
	@Test
	public void Keys_values_entry(){ 
		java.util.Set<Integer> claves= new java.util.HashSet<Integer>();
		java.util.Set<Integer> valores= new java.util.HashSet<Integer>();
		java.util.LinkedHashMap<Integer, Integer> entradas;
		...
		Random r = new Random();
		entradas = new java.util.LinkedHashMap<Integer,Integer>();
